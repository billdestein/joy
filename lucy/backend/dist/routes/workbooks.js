...
import { userDir, workbookDir, picFile, readWorkbook, writeWorkbook, } from '../fileSystem.js';
...
workbooksRouter.post('/create-workbook', async (req, res) => {
    ...
    const workbook = { workbookName, pics: [], prompts: [] };
...
workbooksRouter.post('/generate-pic', async (req, res) => {
    ...
    const newPic = { createdAt: Date.now(), filename: 'unnamed', mimeType };
    const updated = { ...workbook, pics: [...workbook.pics, newPic] };
    await writeWorkbook(user.slug, workbook.workbookName, updated);
...
workbooksRouter.get('/get-workbook', async (req, res) => {
    ...
    if (!user)
        return;
    const workbookName = req.query.workbookName;
...
workbooksRouter.post('/rename-pic', async (req, res) => {
    ...
    const updated = {
        ...workbook,
        pics: workbook.pics.map((p) => p.filename === 'unnamed' ? { ...p, filename: newPicName } : p),
    };
    await writeWorkbook(user.slug, workbook.workbookName, updated);
    res.sendStatus(200);
});
